...
pub fn cmd_add_task(name: String, description: Option<String>, enddate: Option<String>) -> Task {
    let mut task_name: String =
        utils::verfify_or_ask_for_value(Some(name), "Task name".to_string(), false);

    let mut task_description: String =
        utils::verfify_or_ask_for_value(description, "Task description".to_string(), true);

    let mut deadline: String =
        utils::verfify_or_ask_for_value(enddate, "Task deadline".to_string(), true);
...
pub fn cmd_update_task(
    task_id: i32,
    task_title: Option<String>,
    task_description: Option<String>,
    _task_date: Option<String>,
) -> bool {
...
pub fn cmd_unknown() {
    println!(
        "{}",
        "Could not parse the command, type --help for help"
            .to_string()
            .red()
            .bold()
    );
